...
def result(request):
    ...
    # Square feet of house is equal to Square feet of house apart from the basement + Square feet of basement

    if (var_l < var_m) or (var_m == 0):

        predict1 = getModel.predict(np.array(
            [var_a, var_b, var_c, var_d, var_e, var_f, var_g, var_h, var_i, var_j, var_k, var_l, var_m, var_n, var_o,
             var_p, var_q, var_r]).reshape(1, -1))
